...
class MasterNodeWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Available Attacks Section - cards
        attacks_widget = QWidget()
        attacks_layout = QHBoxLayout(attacks_widget)
        attacks_layout.setSpacing(20)
        # BruteForce card
        brute_box = QGroupBox("BruteForce Attack")
        brute_box.setStyleSheet(get_groupbox_style(SUCCESS_GREEN, SUCCESS_GREEN))
        brute_layout = QVBoxLayout(brute_box)
        brute_desc = QLabel("SSH/HTTP/FTP password brute force. Configure in dashboard.")
        brute_desc.setStyleSheet(f"color: {TEXT_SECONDARY}; font-size: 13px; font-family: {FONT_UI};")
        brute_layout.addWidget(brute_desc)
        brute_deploy = QPushButton("Deploy")
        brute_deploy.setStyleSheet(get_primary_button_style(ACCENT_BLUE))
        brute_deploy.setToolTip("Deploy BruteForce attack")
        brute_deploy.clicked.connect(lambda: self.deploy_attack("BruteForce"))
        brute_layout.addWidget(brute_deploy)
        attacks_layout.addWidget(brute_box)
        # DoS card
        dos_box = QGroupBox("DoS Attack")
        dos_box.setStyleSheet(get_groupbox_style(DANGER_RED, DANGER_RED))
        dos_layout = QVBoxLayout(dos_box)
        dos_desc = QLabel("Denial of Service (SYN/UDP/ICMP flood). Configure in dashboard.")
        dos_desc.setStyleSheet(f"color: {TEXT_SECONDARY}; font-size: 13px; font-family: {FONT_UI};")
        dos_layout.addWidget(dos_desc)
        dos_deploy = QPushButton("Deploy")
        dos_deploy.setStyleSheet(get_danger_button_style())
        dos_deploy.setToolTip("Deploy DoS attack")
        dos_deploy.clicked.connect(lambda: self.deploy_attack("DoS"))
        dos_layout.addWidget(dos_deploy)
        attacks_layout.addWidget(dos_box)
        # Wordlist Generator card
        wordlist_box = QGroupBox("Wordlist Generator")
        wordlist_box.setStyleSheet(get_groupbox_style("#f59e0b", "#f59e0b"))
        wordlist_layout = QVBoxLayout(wordlist_box)
        wordlist_desc = QLabel("Generate custom password lists from patterns, length, rules, and character sets.")
        wordlist_desc.setStyleSheet(f"color: {TEXT_SECONDARY}; font-size: 13px; font-family: {FONT_UI};")
        wordlist_layout.addWidget(wordlist_desc)
        wordlist_deploy = QPushButton("Deploy")
        wordlist_deploy.setStyleSheet(get_success_button_style())
        wordlist_deploy.setToolTip("Open the Wordlist Generator tool")
        wordlist_deploy.clicked.connect(lambda: (self.deploy_attack("WordlistGenerator"), self.show_section(1)))
        wordlist_layout.addWidget(wordlist_deploy)
        attacks_layout.addWidget(wordlist_box)
        attacks_layout.addStretch()
        self.sections.addWidget(attacks_widget)
        dashboard_widget = QWidget()
        self.dash_layout = QVBoxLayout(dashboard_widget)
        self.dash_group = QGroupBox("Attack Dashboard")
        self.dash_group.setStyleSheet(get_groupbox_style(ACCENT_CYAN))
        self.dash_group_layout = QVBoxLayout(self.dash_group)
        self.attack_info = QLabel("No attack deployed.")
        self.attack_info.setStyleSheet(f"font-size: 14px; color: {TEXT_PRIMARY}; background: {BG_INPUT}; border-radius: 8px; padding: 12px 16px; font-family: {FONT_UI};")
        self.dash_group_layout.addWidget(self.attack_info)
        self.config_form = QWidget()
        self.config_form_layout = QGridLayout(self.config_form)
        self.dash_group_layout.addWidget(self.config_form)
        btn_row = QHBoxLayout()
        btn_row.setSpacing(12)
        self.start_btn = QPushButton("Start Attack")
        self.start_btn.setStyleSheet(get_success_button_style())
        self.start_btn.setToolTip("Start the configured attack")
        self.start_btn.clicked.connect(self.start_attack)
        self.start_btn.setEnabled(False)
        btn_row.addWidget(self.start_btn)
        self.stop_btn = QPushButton("Stop Attack")
        self.stop_btn.setStyleSheet(get_danger_button_style())
        self.stop_btn.setToolTip("Stop the running attack")
        self.stop_btn.clicked.connect(self.stop_attack)
        self.stop_btn.setEnabled(False)
        btn_row.addWidget(self.stop_btn)
        self.distribute_btn = QPushButton("Distribute")
        self.distribute_btn.setStyleSheet(get_primary_button_style("#f59e0b", "#fbbf24"))
        self.distribute_btn.setToolTip("Distribute attack tasks to connected worker nodes")
        self.distribute_btn.clicked.connect(self.distribute_tasks)
        self.distribute_btn.setEnabled(False)
        btn_row.addWidget(self.distribute_btn)
        btn_row.addStretch()
        self.dash_group_layout.addLayout(btn_row)
        self.dash_group_layout.addStretch()
        self.dash_layout.addWidget(self.dash_group)
        self.dash_layout.addStretch()
        self.sections.addWidget(dashboard_widget)
        ...
        self.main_layout.addWidget(self.sections)
        sep2 = QFrame()
        sep2.setFrameShape(QFrame.HLine)
        sep2.setStyleSheet(f"background: {BORDER_SUBTLE}; max-height: 1px;")
        ...
        self.main_layout.addWidget(attack_group, stretch=2)
    ...
    def show_section(self, idx):
        ...
        btns = [self.attacks_btn, self.dashboard_btn, self.settings_btn, self.workers_btn]
        sections = ["Attacks", "Dashboard", "Settings", "Workers"]
    ...
    def deploy_attack(self, attack_type):
        ...
        self.attack_info.setText(f"<b>Selected Attack:</b> {attack_type}")
        self.start_btn.setEnabled(True)
        self.distribute_btn.setEnabled(True)
        self.show_attack_config(attack_type)

    def show_attack_config(self, attack_type):
        # Remove old widgets
        for i in reversed(range(self.config_form_layout.count())):
            widget = self.config_form_layout.itemAt(i).widget()
            if widget:
        ...
        if attack_type == "BruteForce":
            ...
            self.bf_target.setPlaceholderText("e.g. 192.168.1.100 or example.com")
            self.bf_target.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Target IP / Domain:"), 0, 0)
            self.config_form_layout.addWidget(self.bf_target, 0, 1)
            # Port
            self.bf_port = QLineEdit()
            self.bf_port.setPlaceholderText("e.g. 22")
            self.bf_port.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Port:"), 1, 0)
            self.config_form_layout.addWidget(self.bf_port, 1, 1)
            # Protocol
            self.bf_protocol = QComboBox()
            self.bf_protocol.addItems(["SSH", "FTP", "HTTP"])
            self.bf_protocol.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Protocol:"), 2, 0)
            self.config_form_layout.addWidget(self.bf_protocol, 2, 1)
            # Username/Email
            self.bf_user = QLineEdit()
            self.bf_user.setPlaceholderText("Username or Email")
            self.bf_user.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Username / Email:"), 3, 0)
            self.config_form_layout.addWidget(self.bf_user, 3, 1)
            # Attack Type
            self.bf_attack_type = QComboBox()
            self.bf_attack_type.addItems(["Dictionary", "Incremental", "Hybrid"])
            self.bf_attack_type.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Attack Type:"), 4, 0)
            self.config_form_layout.addWidget(self.bf_attack_type, 4, 1)
            # Upload Wordlist
            self.bf_wordlist = QLineEdit()
            self.bf_wordlist.setReadOnly(True)
            self.bf_wordlist.setStyleSheet(input_style)
            self.bf_wordlist_btn = QPushButton("Browse...")
            self.bf_wordlist_btn.setStyleSheet(get_secondary_button_style())
            self.bf_wordlist_btn.clicked.connect(self.browse_wordlist)
            wordlist_row = QHBoxLayout()
            wordlist_row.addWidget(self.bf_wordlist)
            wordlist_row.addWidget(self.bf_wordlist_btn)
            wordlist_widget = QWidget()
            wordlist_widget.setLayout(wordlist_row)
            self.config_form_layout.addWidget(QLabel("Upload Wordlist:"), 5, 0)
            self.config_form_layout.addWidget(wordlist_widget, 5, 1)
            # Number of Threads
            self.bf_threads = QSpinBox()
            self.bf_threads.setMinimum(1)
            self.bf_threads.setMaximum(1000)
            self.bf_threads.setValue(10)
            self.bf_threads.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Number of Threads:"), 6, 0)
            self.config_form_layout.addWidget(self.bf_threads, 6, 1)
            # Timeout
            self.bf_timeout = QSpinBox()
            self.bf_timeout.setMinimum(1)
            self.bf_timeout.setMaximum(60)
            self.bf_timeout.setValue(5)
            self.bf_timeout.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Timeout (sec):"), 7, 0)
            self.config_form_layout.addWidget(self.bf_timeout, 7, 1)
            # Success Indicator
            self.bf_success_indicator = QLineEdit()
            self.bf_success_indicator.setPlaceholderText("e.g. Welcome, Dashboard, etc.")
            self.bf_success_indicator.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Success Indicator (text in response):"), 8, 0)
            self.config_form_layout.addWidget(self.bf_success_indicator, 8, 1)
        elif attack_type == "DoS":
            # Target IP/Domain
            self.dos_target = QLineEdit()
            self.dos_target.setPlaceholderText("e.g. 192.168.1.100 or example.com")
            self.dos_target.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Target IP / Domain:"), 0, 0)
            self.config_form_layout.addWidget(self.dos_target, 0, 1)
            # Port
            self.dos_port = QLineEdit()
            self.dos_port.setPlaceholderText("e.g. 80")
            self.dos_port.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Port:"), 1, 0)
            self.config_form_layout.addWidget(self.dos_port, 1, 1)
            # Attack Method
            self.dos_method = QComboBox()
            self.dos_method.addItems(["TCP Flood", "UDP Flood", "HTTP Flood", "SYN Flood"])
            self.dos_method.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Attack Method:"), 2, 0)
            self.config_form_layout.addWidget(self.dos_method, 2, 1)
            # Number of Threads
            self.dos_threads = QSpinBox()
            self.dos_threads.setMinimum(1)
            self.dos_threads.setMaximum(1000)
            self.dos_threads.setValue(10)
            self.dos_threads.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Number of Threads:"), 3, 0)
            self.config_form_layout.addWidget(self.dos_threads, 3, 1)
            # Packet Size
            self.dos_packet_size = QSpinBox()
            self.dos_packet_size.setMinimum(64)
            self.dos_packet_size.setMaximum(65535)
            self.dos_packet_size.setValue(1024)
            self.dos_packet_size.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Packet Size (bytes):"), 4, 0)
            self.config_form_layout.addWidget(self.dos_packet_size, 4, 1)
            # Interval
            self.dos_interval = QSpinBox()
            self.dos_interval.setMinimum(1)
            self.dos_interval.setMaximum(10000)
            self.dos_interval.setValue(100)
            self.dos_interval.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Interval (ms):"), 5, 0)
            self.config_form_layout.addWidget(self.dos_interval, 5, 1)
            # Attack Duration
            self.dos_duration = QSpinBox()
            self.dos_duration.setMinimum(1)
            self.dos_duration.setMaximum(3600)
            self.dos_duration.setValue(60)
            self.dos_duration.setStyleSheet(input_style)
            self.config_form_layout.addWidget(QLabel("Attack Duration (sec):"), 6, 0)
            self.config_form_layout.addWidget(self.dos_duration, 6, 1)
        elif attack_type == "WordlistGenerator":
            # Wordlist Generator tool - configure in dashboard when implemented
            pass
            # If you want to open a separate window, uncomment the line below
            # from PyQt5.QtWidgets import QApplication
            # app = QApplication(sys.argv)
            # win = WordlistGeneratorWindow() # Assuming WordlistGeneratorWindow is a separate class
            # win.show()
            # sys.exit(app.exec_())
    ...
    def start_attack(self):
        if not self.selected_attack:
            return
        if self.selected_attack == "BruteForce":
            # Collect parameters from form
            target = self.bf_target.text().strip()
            port = self.bf_port.text().strip()
            protocol = self.bf_protocol.currentText().lower()
            username = self.bf_user.text().strip()
            wordlist = self.bf_wordlist.text().strip()
            attack_type = self.bf_attack_type.currentText().lower()
            threads = self.bf_threads.value()
            timeout = self.bf_timeout.value()
            success_indicator = self.bf_success_indicator.text().strip()
            if protocol == "http":
                self.is_attack_running = True
                self.start_btn.setEnabled(False)
                self.stop_btn.setEnabled(True)
                self.distribute_btn.setEnabled(True)
                self.log_attack(f"[Attack '{self.selected_attack}' started]")
                t = threading.Thread(target=self.run_brute_force_http_gui, args=(target, port, username, wordlist, timeout, success_indicator))
                t.start()
                return
        # fallback to old simulation for other attacks
        self.is_attack_running = True
        self.start_btn.setEnabled(False)
        self.stop_btn.setEnabled(True)
        self.distribute_btn.setEnabled(True)
        self.log_attack(f"[Attack '{self.selected_attack}' started]")
        self.simulate_log_timer = QTimer()
        self.simulate_log_timer.timeout.connect(self.simulate_log_update)
        self.simulate_log_timer.start(1000)
