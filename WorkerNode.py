...
from PyQt5.QtWidgets import (
    QApplication, QMainWindow, QLabel, QPushButton, QVBoxLayout, QHBoxLayout, QWidget, QLineEdit, QTextEdit, QGroupBox, QCheckBox, QFileDialog, QFrame, QScrollArea
)
...
class WorkerNodeWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.pin_input.setMaximumWidth(140)
    ...
    def connect_to_master(self):
        ...
        try:
            ...
            s.close()
            if resp.strip() != b'OK':
                self._set_status("pin_incorrect")
                return
        except Exception:
            self._set_status("pin_failed")
            return
        if self.log_client:
            self.log_client.stop()
        self.log_client = LogClient(ip, port=50050, on_log=self.append_log, worker_name=worker_name)
        if self.log_client.start_client():
            self._set_status("connected")
            if hasattr(self, 'worker_status_text'):
                self.worker_status_text.setText(f"Connected to {ip}")
        else:
            self._set_status("connection_failed")
            if hasattr(self, 'worker_status_text'):
                self.worker_status_text.setText("Connection failed")
